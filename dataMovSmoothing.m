...
target_folder = 'mov_smoothed_dataset';
...
for subfolder_idx = 1 : size(subfolders, 1)
   ...
   %Static case
   out_data.static.power = movSmoother( data.static.power );
   out_data.static.phase = movSmoother( data.static.phase );
   out_data.static.timestamps = data.static.timestamps;
   
   %dynamic case
   for orientation = orientations
        for dynamicCase = dynamicCases
            out_data.dynamic.(orientation).(dynamicCase).power = movSmoother( data.dynamic.(orientation).(dynamicCase).power );
            out_data.dynamic.(orientation).(dynamicCase).phase = movSmoother( data.dynamic.(orientation).(dynamicCase).phase );
